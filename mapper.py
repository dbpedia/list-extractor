    ...
    for res_key in resDict.keys():  # iterate on resource dictionary keys
        ...
        for dk in domain_keys:  # search for resource keys related to the selected domain
            ...
            if not mapped and re.search(dk, res_key, re.IGNORECASE):
                ...
                res_elems += eval(
                    mapper)  # calls the proper mapping for that domain and gets the number of extracted elements
                mapped = True  # prevents the same section to be mapped twice
    mapped_domains = []
    return res_elems
...
def filmpart_mapper(sect_name, lang):
    """
    Returns the part the person took in that movie as a property: starring, director, etc...
